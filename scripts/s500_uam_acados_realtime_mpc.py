...
class AcadosFullStateRealtimeMPC:
    ...
    def _build_solver(self, *, max_iter: int):
        ...
        # check_reuse_possible=False: force codegen/build so a rebuild always reflects
        # the current cost W / options instead of silently reusing cached compiled code.
        try:
            ...
            self.solver = AcadosOcpSolver(ocp, json_file=str(code_export_dir / "ocp.json"))
    ...
    def solve_step(
        ...
        self.solver.set(N, "yref", self._state_to_y(x_ref_N))
        ...
        # Solver diagnostics (exposed for the /mpc_stats topic).
